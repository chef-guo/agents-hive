...
Chat 会话底部的 KB 栏支持把多个 namespace 一次性绑定到当前会话；已绑定 namespace 会显示为可移除的标签。绑定结果会写入服务端 session KB bindings，Agent 检索时仍由 `KBBindingResolver` 按当前 user/session/domain 解析授权集合，不依赖前端传入 namespace 作为权限凭证。
